...
from NeuRIS.utils.utils_image import read_images, write_image, write_images
...
'''
intrinsics (4, 4)
array([[ 5.99805847e+02, -3.77269345e-04,  3.19125793e+02, 0.00000000e+00],
       [ 0.00000000e+00,  5.98515137e+02,  2.43039749e+02, 0.00000000e+00],
       [ 0.00000000e+00,  0.00000000e+00,  1.00000000e+00, 0.00000000e+00],
       [ 0.00000000e+00,  0.00000000e+00,  0.00000000e+00, 1.00000000e+00]])
pose (4, 4)
array([[-0.9981344 , -0.02582536, -0.05532521, -0.13210782],
       [ 0.04389001,  0.32642683, -0.94420296,  0.18159889],
       [ 0.04244401, -0.9448696 , -0.32468438,  0.07230387],
       [ 0.        ,  0.        ,  0.        ,  1.        ]],
      dtype=float32)
'''
...
class Dataset:
    ...
    def __init__(self, conf, args):
        ...
        for ext in ['.png', '.JPG']:
            ...
            self.vec_stem_files = get_files_stem(f'{self.data_dir}/image', ext_file=ext) # self.vec_stem_files 图像名 ['0000', '0006' ...]
            if len(images_lis) > 0:
                break
        ...
        # 读取图像的边缘图
        self.edge = []
        edges_lis = sorted(glob(os.path.join(self.data_dir, f'pred_edge/*.png')))
        self.edges_np = np.stack([self.read_img(im_name, self.resolution_level) for im_name in edges_lis]) / 256.0
        self.edges = torch.from_numpy(self.edges_np.astype(np.float32)).to(self.device)
    ...
    def read_img(self, path, resolution_level):
        img = cv.imread(path)
        H, W = img.shape[0], img.shape[1] # 数据集图像尺寸
    ...
    def get_pose(self, img_idx, pose):
        pose_cur = None
        if pose == None:
            pose_cur = self.pose_all[img_idx] # pose_all torch.Size([367, 4, 4])
        ...
        return pose_cur.squeeze() # torch.Size([4, 4])
    '''
    tensor([[ 0.9959,  0.0613,  0.0663, -0.0530],
            [-0.0421, -0.3345,  0.9415, -0.0841],
            [ 0.0799, -0.9404, -0.3305,  0.0481],
    ...
    def random_get_rays_at(self, img_idx, batch_size, pose = None):
        ...
        # ------------------------------------------------option0: 均匀采样----------------------------------------------------------------
        if not self.args.is_sdf_edge:

            pixels_x = torch.randint(low=0, high=self.W, size=[batch_size]) # 随机选择512个射线512*512 像素面随机取512点
            pixels_y = torch.randint(low=0, high=self.H, size=[batch_size])

        # ------------------------------------------------option1: 边缘重要性采样----------------------------------------------------------------
        else:
            edge = self.edges[img_idx]
            h_img, w_img, _ = self.images[img_idx].shape
            # pixels_y_edge, pixels_x_edge = np.nonzero(edge.numpy().sum(axis=-1))
            # pixels_y_edge, pixels_x_edge = torch.from_numpy(pixels_y_edge,), torch.from_numpy(pixels_x_edge)
            nonzero_indices = torch.nonzero(edge.sum(dim=-1))
            pixels_y_edge, pixels_x_edge = nonzero_indices[:, 0], nonzero_indices[:, 1]
            # edge_sample = int(2 * batch_size * pixels_x_edge.shape[0] / (h_img * w_img))
            edge_sample = int(batch_size * pixels_x_edge.shape[0] / (h_img * w_img))
            edge_index = (torch.randint(low=0, high=pixels_x_edge.shape[0], size=[int(edge_sample)]))
            # print(edge_index.device)
            pixels_y_edge, pixels_x_edge = pixels_y_edge[edge_index].to(self.device), pixels_x_edge[edge_index].to(self.device)
            pixels_x_ori = torch.randint(low=0, high=self.W, size=[int(batch_size - edge_sample)])
            pixels_y_ori = torch.randint(low=0, high=self.H, size=[int(batch_size - edge_sample)])
            pixels_y, pixels_x = (torch.cat((pixels_y_edge, pixels_y_ori), dim=0)), (torch.cat((pixels_x_edge, pixels_x_ori), dim=0))
        #---------------------------------------------------------------------------------------------------------------------------------------
